...
class ProjectGuardTests(unittest.TestCase):
    def test_model_context_cannot_bypass_budget_or_unknown_guard(self):
        self.assertIsNone(w.guarded_route('simple', 0))
        self.assertIsNone(w.guarded_route('abstain', 100))
        self.assertEqual(w.guarded_route('simple', 2), 'small')
